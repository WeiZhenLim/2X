...
def check_export_count(data, col_list=['Campaign Group ID', 'Campaign ID', 'Ad ID']):
    ...
    result = data[col_list].apply(lambda col: col.unique())
...
def preprocess_ads_performance_report(data):
    ...
    3. Extract website and utm parameters from url (And Remove Ori URL Column).
    4. Staging remap based on campaign objectives.
    ...
    req_col = ["Account Name", 
               ...
               "Cost Type", "Daily Budget", "Total Budget", 
               "Creative Name", "Ad ID", "Creative Status", "Ad Introduction Text", "Ad Headline", "Ad Line", 
               "Click URL", "Sponsored Update Type", "DSC Name", "Video Length (in Seconds)"]
    data = data[req_col]
...
if __name__ == "__main__":
    ...
    check_export_count(df_report)
    check_export_count(df_bulk, ['Campaign Group ID', 'Campaign ID', 'Creative ID'])